...
SELECT
  p.id, p.customer_id, p.salesperson_id, p.user_id, p.currency,
  p.quoted_price, p.actual_cost, p.customer_deposit, p.status, p.notes,
  p.created_at, p.updated_at,
  c.code AS customer_code,
  s.name AS salesperson_name,
  COALESCE((
    SELECT SUM(
      COALESCE(t.theoretical_cost,
        CASE
  ...
  GREATEST(0, COALESCE(p.quoted_price, 0)
    - COALESCE((
      SELECT SUM(
        COALESCE(t.theoretical_cost,
          CASE
